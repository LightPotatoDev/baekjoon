...
int main(){
    ...
    for (int i=0; i<m; i++){
        ...
        if (q == 1){
            cin >> left >> right;
            update_range(tree, lazy, 0, n-1, 1, left-1, right-1, 1);
            if (right != n){
                update_range(tree, lazy, 0, n-1, 1, right, right, -(right-left+1));
            }
        }
        else if (q == 2)
        {
            cin >> right;
            cout << get_sum(tree, lazy, 0, n-1, 1, 0, right-1, 0) << '\n';
        }
